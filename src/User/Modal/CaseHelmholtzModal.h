...
class CaseHelmholtzModal : public CaseBase<TCase>
{
    // 求解Helmholtz方程
...
public:
    ...
    double q = 0.0001;                   // 声源强度

    TCase freqFrom = 50, freqTo = 400, freqDelta = 5; // 计算模态时使用，拟合时使用
public:
    // 场
...
public:
    CaseHelmholtzModal(Mesh3<TCase> *_pMesh)
    ...
    EquationHelmholtzModal<TCase, complex<TCase>> *getE_H()
    {
        return (EquationHelmholtzModal<TCase, complex<TCase>> *)(this->GetEqByName(tag_E_H));
    }
...
public:
    ...
    void Define()
    {
        this->Name = "Helmholtz方程 计算Case:Modal Analysis";
        this->Stability = true;  // 计算稳态方程
        this->_bUseChem = false; // 无反应
        this->_DeltaT = 1;       // 1e-3;

        // 复数压力场
        function<void()> _p_updata = bind(&CaseHelmholtzModal::fn_p_updata, this);
        ...
        // 声源项
        function<void()> _q_updata = bind(&CaseHelmholtzModal::fn_q_updata, this);
        ...
        // 定义方程
        this->equations[tag_E_H] = make_shared<EquationHelmholtzModal<TCase, complex<TCase>>>(this);
    }
    ...
    void InitFields()
    {
        ...
        //
        // this->_Mesh->template outputTecPlotFEM<double>("d:/pp.plt", "cccc", Q->fValues->data);
        ...
        for (long bcID : this->_Mesh->FaceBD)
        {
            ...
            if (bcID == 110)
            {
                ...
                long bfidTo = bc->end;
                shared_ptr<Impedance<TCase, complex<TCase>>> imp = make_shared<Impedance<TCase, complex<TCase>>>(3, bc->length, bc->start);

                // 计算系数
                imp->CalcuImpCoeffs = [this, bfidFrom, bfidTo](Impedance<TCase, complex<TCase>> *_imp) -> void
                {
                    stopWatch();
                    ...
                    complex<TCase> *x = new complex<TCase>[npts];
                    complex<TCase> *zz = new complex<TCase>[npts];
                    {
                        int i = 0;
                        for (TCase fHz = freqFrom; fHz <= freqTo; fHz += freqDelta)
                        {
                            TCase k = 2 * PI * fHz / c0;
                            complex<TCase> z = 1.0 / sigma * (7.337e-3 * (1 + 72.23 * Ma) + 1i * 2.2245e-5 * (1 + 51 * t) * (1 + 204 * d) * fHz);
                            zz[i] = z;
                            x[i] = k;
                            i++;
                        }
                    ...
                    {
                        complex<TCase> *y = new complex<TCase>[npts];
                        int i = 0;
                        for (TCase fHz = freqFrom; fHz <= freqTo; fHz += freqDelta)
                        {
                            auto k = x[i];
                            complex<TCase> z = zz[i];
                            TCase delta = this->_Mesh->dcf[faceId];
                            y[i] = k / (1i * z - k * delta);
                            i++;
                        }
                        // 拟合
