...
public:
	...
	vector3D(const T *f, long id) : x(f[id * 3]), y(f[id * 3 + 1]), z(f[id * 3 + 2]) {}
	// destructor
	~vector3D() = default;

	inline void SetToPointer(T *f, long id)
	{
		f[id] = x;
		f[id + 1] = y;
		f[id + 2] = z;
	...
	inline void load(const T &X, const T &Y, const T &Z)
	{
		x = X;
		y = Y;
		z = Z;
	...
	// v[n]   (get method)
	inline T &operator[](const std::size_t n)
	{ // get method
		switch (n)
		{
		case 0:
	...
	template <typename U>
	inline auto angle(const vector3D<U> &V) const
	{ // angle method
...
public:
	...
	inline void load(const T &X, const T &Y)
	{
		x = X;
		y = Y;
	} // set method
	...
	// v.angle(w) more precise than acos(dot(v,w) / (norm2(v)))
	template <typename U>
	inline auto angle(const vector2D<U> &V) const
	{ // angle method
		if (this->norm2() == 0 || V.norm2() == 0)
			throw std::invalid_argument("zero-vector error");
